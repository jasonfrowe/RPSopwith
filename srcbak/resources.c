...
static void draw_gauge_2tiles(uint8_t x, uint16_t value, uint16_t max_value, uint8_t base_tile)
{
    ...
    fill16 = ((uint32_t)value * 16u + ((uint32_t)max_value / 2u)) / (uint32_t)max_value;
    if (fill16 > 16u) {
        fill16 = 16u;
    }
...
bool resources_tick_10hz(bool at_home, bool airborne, bool crashed, bool falling, uint8_t speed)
{
    ...
    if (at_home && !crashed) {
        ...
        {
            uint16_t bombs_u16 = s_resources.bombs;
            uint16_t missiles_u16 = s_resources.missiles;
