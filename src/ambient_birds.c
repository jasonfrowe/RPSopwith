...
static void tick_bird(ambient_bird_t *bird)
{
...

    {
        ...
        int16_t y_limit = (int16_t)(terrain_y - BIRD_GROUND_CLEARANCE_Y -
                                    (PROJECTILE_SPRITE_SIZE_PX - 1));
                                   ...
                                   uint8_t half_width_px, uint8_t half_height_px)
{
    ...
    for (i = 0; i < MAX_BIRD_SPRITES; ++i) {
        ...
        if (bird_box_intersects(world_x, center_y, half_width_px, half_height_px, &s_birds[i])) {
            return hit_bird(i);
        }
    }
